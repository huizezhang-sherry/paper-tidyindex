...
dim_weights <- gggi_weights %>%
  filter(!is.na(pillar_weight)) %>%
  select(variable, pillar_weight) %>% 
  bind_cols(rand_w)

dt <- gggi %>% 
  select(country:political_empowerment) %>% 
  init(id = country) %>%
  add_meta(dim_weights, var_col = variable)
...
```{r gggi-weights, fig.height=20, fig.align='center', fig.width=10}
dt3 <- dt2 %>%
...
```{r idx-tour, fig.align='center', fig.height= 12, fig.width=12}
