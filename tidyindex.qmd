...
The tidy framework consists of two key components: tidy data and tidy tools. The concept of tidy data [@wickham_tidy_2014] prescribes specific rules for organising data in an analysis, with observations as rows, variables as columns, and types of observational units as tables. Tidy tools, on the other hand, are concatenated to establish a sequential process through which the tidy data object flows, creating a pipeline for data processing. In this workflow, all the functions take inputs and outputs as tidy data, enabling direct concatenation of functions and saving analysts from bridging the outputs to subsequent operations. Additionally, the pipeline workflow breaks down complex problems into smaller, more manageable pieces. In an interactive data analysis, this flexibility to link various operations together as needed makes the pipeline workflow preferable over a monolithic design, where all the steps are predetermined and tightly integrated into a single piece. 


<!-- **What is pipeline, its underlying software design philosophy, and how these are reflected in R** -->

Dating back to earlier times, the pipeline approach to structure data analysis draws inspiration from the interactive graphics literature [@buja_elements_1988; @sutherland_orca_2000; @xie_reactive_2014; @wickham_plumbing_2009]. More recently, the tidy framework has provided a conceptual guide for the tidyverse suite [@wickham_welcome_2019] for general-purpose data wrangling, a concept that has gained popularity within the R community. Subsequently, more packages have since been developed to analyze data using the tidy framework for specific domains, for example, `tidytext` for text data [@silge_tidytext_2016], and `naniar` for missing data [@tierney_expanding_2023], and `tidymodels` for building machine learning models [@tidymodels]. To create a tidy workflow tailored to a specific domain, developers first need to identify the fundamental building blocks in the typical workflow. These components are then implemented as modules, which can be combined to form the pipeline. For example, in supervised machine learning models, steps such as data splitting, model training and model evaluation are commonly used in most workflow. In the `tidymodels`, these steps are correspondingly implemented as package `rsample`, `parsnip`, and `yardstick`, agnostic to the specific model chosen. 
