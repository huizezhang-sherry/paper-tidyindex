...
We run a local tour \[TODO: reference to local tour\] to slightly vary the weight of the politics dimension from the original 0.25 while keeping the weights constant for the other three dimensions. 
...
In GGGI, the index value has a direct interpretation as the percentage of gender gap that has been closed. When countries exhibit closer index values, or consistent decreasing index values, it can be directly interpreted as country's progress towards gender parity. Analysts must carefully consider whether such interpretation is reasonable and review the index construction process.
