...
Now, we introduce the notation used for describing pipeline modules. Consider multivariate spatio-temporal data, 

\begin{equation}
\begin{aligned}
\mathbf{x}(s;t) = [x^1(s;t), x^2(s;t), & \cdots, x^P(s;t)] \\
& p = 1, 2, \cdots P, s \in D_s \subseteq \mathbb{R}^d, t \in D_t \subseteq \mathbb{R}^m 
\end{aligned}
\end{equation}

where $x^l$ is the $p$th variable, $s$ is the spatial location, and $t$ is the temporal order. <!-- We assume the spatial and temporal indexes do not depend on the variable, that is, all the variables share the same set of spatial and temporal indexes.--> The spatial space, $D_s \subseteq \mathbb{R}^d$, is generic to include different spatial representations, including a 1D set of $n$ locations $\{D_s: s_1, s_2, \cdots, s_n\}$, a 2D set of longitude and latitude pairs, and $d$-D coordinate set. The higher dimension in the temporal space, $D_t \subseteq \mathbb{R}^m$, can be used to represent for a $m$-D deconstruction of time. For example, a year-month combination, $(Y, M)^\prime$, can be used to record monthly precipitation series from January 1990 to December 2020, where $Y = 1990, \cdots, 2020$ and $M = 1, \cdots, 12$.  To simplify the notation, a subscript letter $s$ or $t$ is used to denote the data have a fixed spatial locations or temporal order. When the spatial locations are fixed, the data become multivariate time series, denoted as $x^l_s(t)$. Similarly,  $x^l_t(s)$ represents multivariate spatial data when the temporal dimension is fixed.
...
`Output`:  $x^l_s(t^{\prime})$, where $t^\prime \in D_{t^{\prime}}$. 
