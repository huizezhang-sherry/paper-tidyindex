...
The tidy framework consists of two key components: tidy data and tidy tools. The concept of tidy data [@wickham_tidy_2014] prescribes specific rules for organising data in an analysis, with observations as rows, variables as columns, and types of observational units as tables. Tidy tools, on the other hand, are concatenated in a sequence through which the tidy object flows, creating a pipeline for data processing. These pipelines are data centric, meaning all the functions take a tidy data object as inputs and return a processed tidy data object, directly ready for the next operations without further reshaping needed. Also, the pipeline approach corresponds to the modular programming practice, which break down complex problems into smaller and more manageable pieces, as oppose to a monolithic design, where all the steps are predetermined and integrated into a single piece. 

<!-- **What is pipeline, its underlying software design philosophy, and how these are reflected in R** -->

Examples of using a pipeline approach to structure data analysis in R can be traced back to the interactive graphics literature in the earlier time [@buja_elements_1988; @sutherland_orca_2000; @xie_reactive_2014; @wickham_plumbing_2009]. @wickham_plumbing_2009 argues that whether made explicit or not, pipeline has to be presented in every graphics program and making them explicit is beneficial for understanding the implementation and comparing between different graphic systems. While this comment is made in the context of graphics program, it is also applicable to other general or domain specific data analysis workflow. More recently, the tidyverse suite [@wickham_welcome_2019] takes the pipeline approach for general-purpose data wrangling and has gained popularity within the R community. The pipeline-style code can be directly read as a series of operations applied successively on the tidy data object, offering a method to document the data wrangling process. Unlike textual documentation, which may omit seemingly obvious or less significant information, the pipeline approach captures all the computational details while retaining the conciseness of a coding language, making it easy for human analysts to comprehend.

Since the success of tidyverse, more packages have been developed to analyze data using the tidy framework for domains specific applications, a noticeable example of which is `tidymodels` for building machine learning models [@tidymodels]. To create a tidy workflow tailored to a specific domain, developers first need to identify the fundamental building blocks in the typical workflow. These components are then implemented as modules, which can be combined to form the pipeline. For example, in supervised machine learning models, steps such as data splitting, model training and model evaluation are commonly used in most workflow. In the `tidymodels`, these steps are correspondingly implemented as package `rsample`, `parsnip`, and `yardstick`, agnostic to the specific model chosen.
...
For constructing indexes, rather than implementing each index separately, as has been done for some indexes, the pipeline approach introduces explicit, user-facing modules that analysts can compose into indexes. This approach provides several advantages to analysts and researchers working on indexes. The index construction process can be complicated to describe and readable pipeline code complements conventional diagrams and textual description for understand the computational details. The uniform interface allows analysts to assemble a diverse array of indexes with a shared set of components, reducing the learning curve for adopting new implementation for different indexes. Importantly, each pipeline module and its corresponding steps are implemented as standalone functions, providing great flexibility for analysis the index. Analysts can rearrange the order in the pipeline or insert new step to experiment with possible new definition for the index or to understand index behaviour. 
...
# Details of the index pipline {#sec-index-pipeline}
