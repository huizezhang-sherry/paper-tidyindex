...
Similar steps used by different indices are difficult to spot since the design of the user interface for indices often includes all the steps under a single function call.
...
While every jumbled index jumbles in its own way, perspicuous indices are all alike. 
...
This work could interests researchers actively developing new indices to deliver new indices in an easy-to-reproduce design. 
...
For statisticians and software developing engineers, this work frames the workflow of index construction and could motivate similar work in other workflow used in practice.
