...
Consider a commonly used drought index: Standardised Precipitation Index (SPI) [@spi]. Its construction involves: 1) aggregating monthly precipitation (`prcp`) with a time scale, 2) fitting the aggregated series with a chosen distribution to get density values, and 3) converting the density values to normal quantiles. Under the pipeline approach, SPI will be constructed as: 

```{r eval = FALSE, echo = TRUE}
DATA %>% 
  aggregate(scale = 12, var = prcp) %>% 
  normalise(dist = gamma, var = prcp, fit_method = lmom) %>% 
...
The pipeline construction produces the same index value as command like `spi(...)`, as shown in @fig-toy-example, but with additional benefit:
...
compare %>% 
  ...
  theme(legend.position = "bottom", 
        aspect.ratio = 1) +
  xlab("Index values from `spi()` in package `SPEI`") + 
  ylab("Index values from pipeline") + 
...
#| fig-align: center
#| fig-cap: "Standardised Precipitation Index (SPI) calculated using two approaches: the pipeline construction (y-axis) and the function `spi()` from the `SPEI` package (x-axis), using the `wichita` data from the package `SPEI`. All the points align diagnally, indicating the same numerical result from both approaches."
#| out-height: 70%
#| out-width: 70%
