...
One of the most commonly known pipeline example is perhaps the Unix pipeline where programs can be concatenated with `|` to flow the output from the last program into the next program. 
The pipeline design avoid writing a complicated program to solve a complex problem but building simple programs that do one thing well and work well together.
This design philosophy is also embraced in the R community by the tidyverse principles. 
In tidyverse, the flow from the previous program to the next is ensured through reusing the tidy data format [@wickham_tidy_2014], which prescribes three rules on how to layout tabular data.
To solve a complicated data problem using tidyverse, one typically builds the solution using a collection of functions and these functions have a strong human centered design with intuitive naming and easy to reference through autocomplete.

**A proper pipeline example in R: tidymodel**

A pipeline example in R that adopts the tidyverse principle is the tidymodel suite [@tidymodels] for building machine learning models. 
