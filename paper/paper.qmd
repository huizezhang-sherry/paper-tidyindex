...
Currently, index construction lacks structure. 
It is often up to the researchers or research institutions to decide whether to provide open source code on the new indices, what would be the best user interface for the new indices, and how easily the new indices can be used by end users, along with other existing indices. 
Institutes like National Oceanic and Atmospheric Administration (NOAA) and United States Drought Monitor (USDM) moderate the indices in their domains.
...
For researchers, the level of varies. Some research groups provide open sourced code available for [explain situation with individual researchers]
This causes the problem that computation on indices are not transparent and code design on indices are done separately.
Researchers who wish to validate the calculation from large institutes need to reinvent the wheels that have already been built.
Open source codes provide narrow margin for adjustment. [more here]
...
Similar steps used by different indices are difficult to spot since they all hide under commands for calculating different indices. 
Lastly, these functions does not support inspecting intermediate results if users wish to check the output of a certain step. 

<!-- users may wish to inspect intermediate results during the index construction to validate the output or to compare results with other indices, but [single command functions] do not support this demand unless the users can customise the source code as they need. -->


<!-- indices are constructed separately in this design. -->
<!-- Many steps are more or less similar when constructing indices, but this would be hard to observe from the current construction.  -->
...
*This week: need to extend each point of why it is beneficial further with more information?*

While jumbled indices jumble in their own ways, perspicuous indices are all alike. 
This paper proposes a data pipeline for index construction. [more on what we bring onto the table: construct indices through pipeline steps where each step corresponds to a single building block???]

The pipeline framework provides an open source tool to construct indices in general. This is beneficial because it opens up the possibility to create, study, and use indices in a more principled way. 
More specifically, it enables researchers to 1) validate the indices calculated from external organisations, 2) unify various indices under the same framework for comparison, 3) swap or adjust individual steps in the index construction to study their contribution, 4) calculate uncertainty  on indices, and finally, 5) inspire proposal of new indices from existing ones. 





<!-- Once a data pipeline is constructed, it opens possibility to do statistics on indices. -->
<!-- For example, bootstrap and other methods could be applied into the pipeline to calculate uncertainty on the index value with randomisation.  -->
<!-- Properties of existing indices can be examined through swapping the steps involved in the construction.  -->
<!-- For example, some indices prefer to standardise the input variables in the beginning of the workflow while other prefer to move the standardisation towards the end and directly apply it on the index value.  -->
<!-- A flexible pipline approach gives the freedom to construct the index in both ways by placing the standardisation step in different stages of the pipeline.  -->
<!-- The importance of the position of a single steps can be compared through indices constructed by placing the step at different stages of the pipeline.  -->
<!-- Also, the pipeline approach motivates new indices through combining existing steps from different indices.  -->
