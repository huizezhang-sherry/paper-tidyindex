...
dt <- wichita %>%
  ...
  init(id = id, time = ym, indicators = PRCP:ACSH) %>%
  var_trans(.method = thornthwaite, Tave = TMED, lat = 37.6475, .new_name = "pet") %>%
  dim_red(diff = PRCP - pet) %>%
...
There is no uniform definition on drought [@ref], a range of drought indexes are usually used simultaneously to quantify the severity of a drought event. 

In the package PACKAGE, a collection of drought indexes have been implemented, including the most commonly used SPI and SPEI (`idx_spi()` and `idx_spei()`). They can be computed collectively on a dataset under `compute_indexes()`: 
...
The individual index functions are convenient wrappers around the pipeline building blocks.

For SPI and SPEI, the wrapper functions also allow for multiple inputs of scales and distribution parameters (`.scale` and `.dist`) to allow computation on multiple time scale and distribution fitting. 

The climate data is sourced from Global Historical Climatology Network Daily (GHCND). 

stations in Queensland, Australia. 
With precipitation, maximum and minimum temperature available since January 1990. 

This gives `r length(unique(queensland$id))` stations.

The following code compute the two indexes (SPI and SPEI) at different time scales (6, 12, 24, 36) and different distributions (log-logistic and general extreme value distributions for SPEI and gamma for SPI). 
...
The output from `compute_indexes()` contains index values and associated parameter in a long tibble. Analysis and visualisation can then be conducted to understand the effect of scale and distribution on the index value or explore the spatial and temporal pattern of wet/dry condition as in @fig-compute-spatial and @fig-compute-temporal.
...
<!-- maybe combine the two figures: Exploratory data analysis on the computed indexes: ??? -->
...
queensland_map %>%
  ...
  scale_color_distiller(palette = "BrBG", direction = 1, name = "Index", ) +
  theme_void() +
