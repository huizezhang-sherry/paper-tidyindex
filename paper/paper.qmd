...
Another type of indexes that are commonly used in social science is to rank entities (i.e. countries) on a topic through a range of indicators. This includes university ranking from QS, Times Higher Education; the Sustainable Development Goals (SDG) Index [@SDSN_Bertelsmann_2021] by the Sustainable Development Solutions Network (SDSN), the Human Development Index (HDI) [@UNDP_2021] by United Nations Development Programme (UNDP), etc. These indexes, or rankings, typically have a set of predefined dimensions, or pillars, and indicators are first reduced to these pillars and then reduced to an index. In the following example, we will first show how to compute the Human Development Index (HDI) using the pipeline developed, and then demonstrate how the alternative expressions and parameters can be tested with two functions: `swap_exprs()` and `swap_values()`.

Human Development Index (HDI) measures the development of a country through [...], more than just the economics. It uses three dimensions and four indicators: health (life expectancy at birth), education (expected years of schooling, mean years of schooling), and standard of living (GNI per capita). The technical notes [@UNDP_2021_tech_notes] from United Nation Development Programme have documented the procedures to calculate HDI and we summarise them as below: 

  - take log on GNI per capita,
  - rescale the four indicators using mini-max with the provided minimum and maximum, 
  - aggregate the two education indicators using the arithmetic mean
  - aggregate the three dimensions into the index using the geometrical mean

After the initial data import and wrangling, these steps can be calculated in the pipeline,as: 
...
The output is an `indri` object where a summary of the operations performed have been provided. The dataset contains the ID, country name,  rescaled variables (`life_exp`, `exp_sch`, `avg_sch`, `gni_pc`), the average of two indicators in the education dimension (`sch`), and the index calculated (`index`).

A set of questions that can interest index analysts are the what-ifs: how would the index/ ranking changes if a particular parameter or expression changes in the pipeline. In the context of HDI, the index is calculated as the geometrical mean of the three dimensions, what if an arithmetic mean or different weights is used? The maximum and minimum limit is carefully chosen with justification provided in the technical notes: 
...
The function `swap_exprs()` and `swap_values()` can be used to answer these what-ifs.  The two functions first uses a set of arguments to locate the operations the analysts would like to test, and then accepts a list of expressions or values for testing, and lastly, the initial `indri` (raw data). Another five expressions with different weighting emphasis are tested on calculating the final HDI: 
...
res2 <- res %>%
  swap_exprs(
    ...
    .expr = list(
      ...
      index5 = 0.569 * life_exp + 0.576 * sch + 0.586 * gni_pc), 
    .raw_data = dt)
```

The result contains another 5 variations of indexes calculated from the expression above. Country ranks can be added using the base function `rank()`. Figure @fig-hdi-expr shows the scatterplot matrix of the country ranking against each other, including the original index (`index0`). From the plot, we learn that the geoemtrical mean (`rank0`), the arithmetic mean (`rank1`), and the PCA recommended weights (`rank5`) give almost similar ranks. This validates the method chosen by the original index. However, when the three dimensions are given different weights (`rank2`, `rank3`, and `rank4`), the country ranking can vary substantially. A cluster of points have been highlighted in the panel `rank3` vs. `rank4`. These countries are ranked highly according to `rank4` but become mediocre when ranked by `index3`. Looking into the three dimensions of these countries in Table @tbl-hdi-expr shows that these countries (Andorra, Qatar, San Marino, Kuwait, and Brunei Darussalam) have high life expectancy and GNI, but relatively low score on education, hence they are ranked lower when education (`sch`) is given a larger weight in `index3` (`0.1 * life_exp + 0.1 * sch + 0.8 * gni_pc`).
