...
For SPI and SPEI, the wrapper functions also allow for multiple inputs of scales and distribution parameters (`.scale` and `.dist`) to allow computation on multiple time scale and distribution fitting. The following code compute the two indexes (SPI and SPEI) for different time scales (6, 12, 18, 24, 30, 36) and different distributions (Pearson III, log-logistic, lognormal, and general extreme value distributions) for [NUMBER] weather stations in Eastern Australia shown in @fig-station-map: (TODO: should talk about the data source)

```{r eval = FALSE}
scale <- c(6, 12, 18, 24, 30, 36)
dist_spei <- c(gev(), loglogistic(), pearsonIII(), lognormal())
dist_spi <- c(gamma(), weibull())
dt %>% 
  compute_indexes(
    spei = idx_spei(
      .pet_method = "thornthwaite", .tavg = tavg, .lat = lat, 
      .scale = scale, .dist = dist_spei),
    spi = idx_spi(.scale = scale,  .dist = dist_spi)
  )
```
