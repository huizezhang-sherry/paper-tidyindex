...
Many variables relevant to decision making cannot be directly measured, however, they are crucial for resource allocation, early prevention, and other operational purpose.
For example, fire authorities would be interested to quantify fire risk since bushfires can have a huge impact on monetary loss, health, and the local ecosystem among others.
Climatologists would be interested in monitoring the change in global conditions since variability in atmospheric and oceanic conditions has a direct impact on global weather and climate.
Usually, more than one variable is related to the concept of interest and they often need to be integrated to make decisions.
...
In many domains, constructing an index is a common approach to characterise these unobservable quantities.
The index approach allows researchers to compare the quantity of interest across countries and also cross time.
...
The meaning of indices varies across disciplines and in this article, an index is defined as a tool to quantify a concept of interest that does not have a direct measure.
The concept of interest doesn't have a direct measure can because it is impractical to measure the concept of interest at the population level. 
...
The lack of direct measure could also be due to the absence of a universal definition on the concept of interest. 
Many natural and social concepts are fundamentally multi-facets and some indices combine measures from different domains to characterise the concept of interest. 
...
Despite many measurements having the term *index* in their name, they cannot be technically classified as indices according to the definition given above. 
A major reason for these measures to lose their index memberships is that the underlying concept of interest is straightforward to measure once defined. 
Sometimes variables like precipitation of the driest month or percentage of days when maximum temperature is below a certain percentile are classified as climate indices, however, precipitation and temperature can be directly measured by rain gauge and thermometer. 
...
Similarly, a set of remote sensing indices should not be classified as indices, but variables, since they are measures of electromagnetic wave reflectance.
...
TODO: clean up terminology:
  ...
  - "measure" for item quantified by instruments or devices in standard units

FYI: 

  - Vegetation Condition Index  = (NDVI - NDVI_min)/ (NDVI_max - NDVI_min)
  - A measure is a variable, the variable itself is not an index. 
  - Not all multivariate "indices" are indices (i.e. those remote sensor transformations), but generally they are. It is possible that an index is constructed from a single variable (i.e. SPI), but be careful on whether it is variable or index.
