...
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[enhanced, sharp corners, breakable, boxrule=0pt, borderline west={3pt}{0pt}{shadecolor}, frame hidden, interior hidden]}{\end{tcolorbox}}\fi
...
distribution fitting. The following code compute the two indexes (SPI
and SPEI) for different time scales (6, 12, 18, 24, 30, 36) and
different distributions (Pearson III, log-logistic, lognormal, and
general extreme value distributions) for {[}NUMBER{]} weather stations
in Eastern Australia shown in Figure~\ref{fig-station-map}: (TODO:
should talk about the data source)

\begin{figure}

{\centering \includegraphics{paper_files/figure-pdf/fig-station-map-1.pdf}

}

\caption{\label{fig-station-map}\ldots{}}
