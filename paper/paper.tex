...
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[borderline west={3pt}{0pt}{shadecolor}, sharp corners, breakable, frame hidden, enhanced, boxrule=0pt, interior hidden]}{\end{tcolorbox}}\fi
...
The construction of natural hazard indexes also follows a set of steps,
which is usually illustrated using a flowchart in the paper.

\begin{itemize}
\tightlist
\item
  constructing time series index should also be encapsulated in the
  framework
\end{itemize}

Here we assume a concept of interest is determined, relevant variables/
indicators are identified and available to construct indexes.

Before the mathmetical construction of indexes
...
With drought indexes, the extraction of meaningful variables from the
