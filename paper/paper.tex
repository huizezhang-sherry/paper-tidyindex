...
researchers or research institutions to decide whether to provide open
source code on the new indices, what would be the best user interface
for the new indices, and how easily the new indices can be used by end
users, along with other existing indices. Institutes like National
Oceanic and Atmospheric Administration (NOAA) and United States Drought
Monitor (USDM) moderate the indices in their domains. They provide
public information to describe the methodology used in the calculation,
publish the index values for public consumption while the source code
used for computing is often not available for public consumption. For
researchers, the level of varies. Some research groups provide open
sourced code available for {[}explain situation with individual
researchers{]} This causes the problem that computation on indices are
not transparent and code design on indices are done separately.
Researchers who wish to validate the calculation from large institutes
need to reinvent the wheels that have already been built. Open source
codes provide narrow margin for adjustment. {[}more here{]} Similar
steps used by different indices are difficult to spot since they all
hide under commands for calculating different indices. Lastly, these
functions does not support inspecting intermediate results if users wish
to check the output of a certain step.
...
all alike. This paper proposes a data pipeline for index construction.
{[}more on what we bring onto the table: construct indices through
pipeline steps where each step corresponds to a single building
block???{]}

The pipeline framework provides an open source tool to construct indices
in general. This is beneficial because it opens up the possibility to
create, study, and use indices in a more principled way. More
...
uncertainty on indices, and finally, 5) inspire proposal of new indices
from existing ones.
