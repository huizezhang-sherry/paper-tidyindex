...
\textbf{Variable transformation } {[}TODO{]}

Variable transformation, and scaling in the next section, are both
pre-processing steps that prepare the data with desire statistical
property to work with. Some desirable property to work with can be a
stable variance, a normal distribution that satisfies modelling
assumptions, or a data range that required by the algorithms. Variable
tranformation is a general notion of a functional transformation on the
variable: \[h_{\tau}(x(\mathbf{s};\mathbf{t})),\] where
...
it is separated into own step to make it explicit in the pipeline. The
key difference between the two steps is that variable transformation
