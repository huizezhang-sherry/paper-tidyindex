...
paramount for data analysis since in practice, data analysis involves
trials and errors on different steps to solve the problems. Analysts
need the freedom to quickly experiment with different ideas when working
with data. The tidyverse suite provides a collection of R package for
conducting data analysis. These tools directly operate on data, and
users, even new users, can easily read from the code on what operation
is performed on the data and what to expect as the output. All of these
are built upon a simple prior knowledge of the tidy data principle
(Wickham 2014), which prescribes three rules for structuring tabular
data: 1) Each variable forms a column, 2) Each observation forms a row,
and; 3) Each type of observational unit forms a table.

Constructing indices also involves this working pattern of trails and
