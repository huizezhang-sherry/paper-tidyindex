...
As a programming language for data analysis, R provides a scripted
environment for users to experiment with data without the complex
abstraction a typical programming language requires. The flexible and
intuitive workflow is paramount for data analysis since in a data
analysis project, the full workflow of how data should be processed from
the raw data to the final results is usually only consolidated till the
end of the project and analysts need the freedom to quickly experiment
with different ideas when working with data. This approach to data
analysis is enhanced by the tidyverse suite where a collection of R
packages are built for data science under the concept of tidy data. The
tidy data principle (Wickham 2014) prescribes three simple rules for
structuring tabular data: 1) Each variable forms a column, 2) Each
observation forms a row, and; 3) Each type of observational unit forms a
table. The packages built in the tidyverse suite provide operations that
are direct actions on the data and users, even new users, can easily
read from the code on what has been performed and what to expect after
each operation. Researchers and analysts have a similar need for an
intuitive and flexible process to construct indices since a similar
workflow occurs where researchers experiment with many different
indices, discover issues with the current ones, and experiment with
potential ideas before finally arrive at a solution.
