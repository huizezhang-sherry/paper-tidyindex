...
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[enhanced, frame hidden, borderline west={3pt}{0pt}{shadecolor}, breakable, boxrule=0pt, sharp corners, interior hidden]}{\end{tcolorbox}}\fi
...
Another type of indexes that are commonly used in social science is to
rank entities (i.e.~countries) on a topic through a range of indicators.
This includes university ranking from QS, Times Higher Education; the
Sustainable Development Goals (SDG) Index (Network and Stiftung 2021) by
the Sustainable Development Solutions Network (SDSN), the Human
Development Index (HDI) (Programme 2021a) by United Nations Development
Programme (UNDP), etc. These indexes, or rankings, typically have a set
of predefined dimensions, or pillars, and indicators are first reduced
to these pillars and then reduced to an index. In the following example,
we will first show how to compute the Human Development Index (HDI)
using the pipeline developed, and then demonstrate how the alternative
expressions and parameters can be tested with two functions:
\texttt{swap\_exprs()} and \texttt{swap\_values()}.

Human Development Index (HDI) measures the development of a country
through {[}\ldots{]}, more than just the economics. It uses three
dimensions and four indicators: health (life expectancy at birth),
education (expected years of schooling, mean years of schooling), and
standard of living (GNI per capita). The technical notes (Programme
2021b) from United Nation Development Programme have documented the
procedures to calculate HDI and we summarise them as below:

\begin{itemize}
\tightlist
\item
  take log on GNI per capita,
\item
  rescale the four indicators using mini-max with the provided minimum
  and maximum,
\item
  aggregate the two education indicators using the arithmetic mean
\item
  aggregate the three dimensions into the index using the geometrical
  mean
\end{itemize}

After the initial data import and wrangling, these steps can be
calculated in the pipeline,as:
...
\NormalTok{(res }\OtherTok{\textless{}{-}}\NormalTok{ dt }\SpecialCharTok{\%\textgreater{}\%}
  ...
  \FunctionTok{var\_trans}\NormalTok{(}\AttributeTok{.method =}\NormalTok{ rescale\_minmax, }\AttributeTok{.vars =}\NormalTok{ life\_exp}\SpecialCharTok{:}\NormalTok{gni\_pc,}
            \AttributeTok{min =}\NormalTok{ scaling\_params}\SpecialCharTok{$}\NormalTok{Minimum, }\AttributeTok{max =}\NormalTok{ scaling\_params}\SpecialCharTok{$}\NormalTok{Maximum) }\SpecialCharTok{\%\textgreater{}\%} 
...
The output is an \texttt{indri} object where a summary of the operations
performed have been provided. The dataset contains the ID, country name,
rescaled variables (\texttt{life\_exp}, \texttt{exp\_sch},
\texttt{avg\_sch}, \texttt{gni\_pc}), the average of two indicators in
the education dimension (\texttt{sch}), and the index calculated
(\texttt{index}).

A set of questions that can interest index analysts are the what-ifs:
how would the index/ ranking changes if a particular parameter or
expression changes in the pipeline. In the context of HDI, the index is
calculated as the geometrical mean of the three dimensions, what if an
arithmetic mean or different weights is used? The maximum and minimum
limit is carefully chosen with justification provided in the technical
notes:
...
The function \texttt{swap\_exprs()} and \texttt{swap\_values()} can be
used to answer these what-ifs. The two functions first uses a set of
arguments to locate the operations the analysts would like to test, and
then accepts a list of expressions or values for testing, and lastly,
the initial \texttt{indri} (raw data). Another five expressions with
different weighting emphasis are tested on calculating the final HDI:
...
\NormalTok{res2 }\OtherTok{\textless{}{-}}\NormalTok{ res }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{swap\_exprs}\NormalTok{(}
    ...
    \AttributeTok{.expr =} \FunctionTok{list}\NormalTok{(}
      ...
      \AttributeTok{index2 =} \FloatTok{0.8} \SpecialCharTok{*}\NormalTok{ life\_exp }\SpecialCharTok{+} \FloatTok{0.1} \SpecialCharTok{*}\NormalTok{ sch }\SpecialCharTok{+} \FloatTok{0.1} \SpecialCharTok{*}\NormalTok{ gni\_pc,}
      \AttributeTok{index3 =} \FloatTok{0.1} \SpecialCharTok{*}\NormalTok{ life\_exp }\SpecialCharTok{+} \FloatTok{0.8} \SpecialCharTok{*}\NormalTok{ sch }\SpecialCharTok{+} \FloatTok{0.1} \SpecialCharTok{*}\NormalTok{ gni\_pc,}
      \AttributeTok{index4 =} \FloatTok{0.1} \SpecialCharTok{*}\NormalTok{ life\_exp }\SpecialCharTok{+} \FloatTok{0.1} \SpecialCharTok{*}\NormalTok{ sch }\SpecialCharTok{+} \FloatTok{0.8} \SpecialCharTok{*}\NormalTok{ gni\_pc,}
      \CommentTok{\# PCA recommended weight}
      \AttributeTok{index5 =} \FloatTok{0.569} \SpecialCharTok{*}\NormalTok{ life\_exp }\SpecialCharTok{+} \FloatTok{0.576} \SpecialCharTok{*}\NormalTok{ sch }\SpecialCharTok{+} \FloatTok{0.586} \SpecialCharTok{*}\NormalTok{ gni\_pc), }
    \AttributeTok{.raw\_data =}\NormalTok{ dt)}
\end{Highlighting}
\end{Shaded}

The result contains another 5 variations of indexes calculated from the
expression above. Country ranks can be added using the base function
\texttt{rank()}. Figure Figure~\ref{fig-hdi-expr} shows the scatterplot
matrix of the country ranking against each other, including the original
index (\texttt{index0}). From the plot, we learn that the geoemtrical
mean (\texttt{rank0}), the arithmetic mean (\texttt{rank1}), and the PCA
recommended weights (\texttt{rank5}) give almost similar ranks. This
validates the method chosen by the original index. However, when the
three dimensions are given different weights (\texttt{rank2},
\texttt{rank3}, and \texttt{rank4}), the country ranking can vary
substantially. A cluster of points have been highlighted in the panel
\texttt{rank3} vs.~\texttt{rank4}. These countries are ranked highly
according to \texttt{rank4} but become mediocre when ranked by
\texttt{index3}. Looking into the three dimensions of these countries in
Table Table~\ref{tbl-hdi-expr} shows that these countries (Andorra,
Qatar, San Marino, Kuwait, and Brunei Darussalam) have high life
expectancy and GNI, but relatively low score on education, hence they
are ranked lower when education (\texttt{sch}) is given a larger weight
in \texttt{index3}
...
\caption{\label{fig-hdi-expr}my plot}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\columnwidth - 14\tabcolsep) * \real{0.1061}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 14\tabcolsep) * \real{0.1061}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 14\tabcolsep) * \real{0.1061}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 14\tabcolsep) * \real{0.0909}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 14\tabcolsep) * \real{0.0909}}@{}}
\caption{\label{tbl-hdi-expr}my tables}\tabularnewline
\toprule()
...
\midrule()
\endhead
Andorra & 0.929 & 0.721 & 0.942 & 0.764 & 0.919 & 69 & 27 \\
Qatar & 0.912 & 0.684 & 1.023 & 0.741 & 0.978 & 83 & 5 \\
San Marino & 0.937 & 0.701 & 0.947 & 0.749 & 0.921 & 76 & 25 \\
Kuwait & 0.903 & 0.670 & 0.947 & 0.721 & 0.915 & 96 & 29 \\
Brunei Darussalam & 0.841 & 0.694 & 0.977 & 0.737 & 0.935 & 85 & 20 \\
\bottomrule()
\end{longtable}
...
\leavevmode\vadjust pre{\hypertarget{ref-SDSN_Bertelsmann_2021}{}}%
Network, Sustainable Development Solutions, and Bertelsmann Stiftung.
2021. {``SDG Index and Dashboards.''} \url{https://sdgindex.org/}.

\leavevmode\vadjust pre{\hypertarget{ref-UNDP_2021}{}}%
Programme, United Nations Development. 2021a. {``Human Development Index
(HDI).''}
\url{https://hdr.undp.org/en/content/human-development-index-hdi}.

\leavevmode\vadjust pre{\hypertarget{ref-UNDP_2021_tech_notes}{}}%
---------. 2021b. {``Human Development Report 2021-2022: The Next
