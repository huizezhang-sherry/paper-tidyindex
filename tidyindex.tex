...
software design
\end{keywords}
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[interior hidden, enhanced, sharp corners, boxrule=0pt, frame hidden, borderline west={3pt}{0pt}{shadecolor}, breakable]}{\end{tcolorbox}}\fi
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.0128}}
...
\midrule()
\endhead
1 & Temporal processing & \(x^i_{s}(t)\) & \(f[x^i_s(t)]\) &
\(\tilde{x}^i_s(t^\prime)\), where \(t^\prime \in D_{t^\prime}\) \\
2 & Spatial processing & \(x^i_t(s)\) & \(g[x^i_t(s)]\) &
\(x^i_t(s^\prime)\), where \(s^\prime \in D_{s^\prime}\) ???? \\
3 & Variable transformation & \(x^i(s; t)\) & \(T[x^i(s;t)]\) &
\(Tx^i(s;t)\) ???? \\
...
the index and implications for interpretation and decision-making. We
use a visualisation method called ``tour'' to animate the data
projection across different sets of weights, aiming to understand how
changes on the politics weight will affect the index value. We select a
set countries for this exercise, which includes:
...
We run a local tour {[}TODO: reference to local tour{]} to slightly vary
the weight of the politics dimension from the original 0.25 while
keeping the weights constant for the other three dimensions.
...
values, or consistent decreasing index values, it can be directly
interpreted as country's progress towards gender parity. Analysts must
carefully consider whether such interpretation is reasonable and review
...
\caption{\label{fig-idx-tour}Six frames selected from varying the
weights of the politics dimension in Global Gender Gap Index (GGGI). The
top panel shows the GGGI value against the country, ranked by its
original index value in Frame 12. The bottom panel displays the weight
used to produce the index value in the top panel, with each frame
corresponds to a set of weights. Countries selected includes 1)
top-ranked countries with GGGI \textgreater{} 0.85, 2) countries ranked
between 57 and 62 with GGGI from 0.72 to 0.73, and 3) low-ranked
countries with GGGI \textless{} 0.6. Compared to Frame 12 where equal
weigts are used for the four dimension, a reduced weight in politics
(Frame 1 and 6) shows narrower gaps between top and mid- or lower-
ranked countries, while an increase in the politics weight (Frame 18,
24, and 29) leads to a systematic decrease of GGGI values across all the
countries, except for Bangladesh. Full animation is available at
