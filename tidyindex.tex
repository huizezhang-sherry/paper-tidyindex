...
software design
\end{keywords}
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[borderline west={3pt}{0pt}{shadecolor}, interior hidden, frame hidden, enhanced, sharp corners, boxrule=0pt, breakable]}{\end{tcolorbox}}\fi
...
\begin{itemize}
\item
  \(x^j(s, j)\) represents a  variable of interest for example precipitaton, $j = 1, \ldots , p$ and 
\item
  \(s\) represents the geographic location in the space
  \(D_s \subseteq \mathbb{R}^m\). Examples of geographic locations include a collection of
  countries, longitude and latitude coordinates or regions of interest and, 
\item
  \(t\) denotes the temporal measurements in \(D_t \subseteq \mathbb{R}^n\).
  For instance, time measurements could be recorded hourly, yearly, monthly, quaterly or by season.
\end{itemize}
...
\midrule()
\endhead
1 & Temporal processing & \(x(t)\) & \(f_{\Psi}[x(.)]\) &
\(x^*(t^\prime)\) where \(t\prime \in D_{t^\prime}\) \\
2 & Spatial processing & \(x(s)\) & \(g_{\Theta}[x(.)]\) &
\(x^\#(s^\prime)\) where \(s^\prime \in D_{s^\prime}\) \\
