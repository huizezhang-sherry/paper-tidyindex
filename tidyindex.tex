...
data flows, creating a pipeline for data processing and modelling. These pipelines
are data centric, meaning all the tidy tools or functions take a tidy data object as
input and return a processed tidy data object, directly ready for the
...
% , as oppose to a monolithic
% design, where all the steps are predetermined and integrated into a
% single piece. 
The flexibility provided by the modularity makes it easier
to modify any step in the pipeline and to maintain and extend the
code base.

Examples of using a pipeline approach for data analysis 
can be traced back to the interactive graphics literature including 
Buja et al. 1988; Sutherland et al. 2000; Xie, Hofmann, and Cheng
2014; Wickham et al. 2009). Wickham et al. (2009) argues that whether
made explicit or not, a pipeline has to be presented in every graphics
...
this comment is made in the context of graphics program (WHAT DO YOU MEAN by graphics program?), it is also
...
operations applied successively on a tidy data object, offering a
method to create a reproducible data wrangling process that is also easy to document.
% Unlike textual documentation, which may omit seemingly
...
separately, as it has been done for some indexes, the pipeline approach
introduces explicit, user-facing (??), and standalone modules for analysts to
assemble indexes. Analysts can then rearrange the pipeline or
insert new steps to create indexes and/or to experiment with possible index definition while also testing
the index behaviour. 
% 
% In addition, the readability of the pipeline code
% can complement conventional diagrams and textual information to
% understand the how indexes are constructed.

(I don't think this is needed -->)The uniform interface of a
