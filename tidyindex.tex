...
object flows, creating a pipeline for data processing. These pipelines
are data centric, meaning all the functions take a tidy data object as
inputs and return a processed tidy data object, directly ready for the
next operations without further reshaping needed. Also, the pipeline
approach corresponds to the modular programming practice, which break
...
Examples of using a pipeline approach to structure data analysis in R
can be traced back to the interactive graphics literature in the earlier
time (Buja et al. 1988; Sutherland et al. 2000; Xie, Hofmann, and Cheng
2014; Wickham et al. 2009). Wickham et al. (2009) argues that whether
made explicit or not, pipeline has to be presented in every graphics
...
this comment is made in the context of graphics program, it is also
applicable to other general or domain specific data analysis workflow.
More recently, the tidyverse suite (Wickham et al. 2019) takes the
pipeline approach for general-purpose data wrangling and has gained
popularity within the R community. The pipeline-style code can be
directly read as a series of operations applied successively on the tidy
data object, offering a method to document the data wrangling process.
Unlike textual documentation, which may omit seemingly obvious or less
significant information, the pipeline approach captures all the
computational details while retaining the conciseness of a coding
language, making it easy for human analysts to comprehend.
...
identify the fundamental building blocks in the typical workflow. These
...
For constructing indexes, rather than implementing each index
separately, as has been done for some indexes, the pipeline approach
introduces explicit, user-facing modules that analysts can compose into
indexes. This approach provides several advantages to analysts and
researchers working on indexes. The index construction process can be
complicated to describe and readable pipeline code complements
conventional diagrams and textual description for understand the
computational details. The uniform interface allows analysts to assemble
a diverse array of indexes with a shared set of components, reducing the
learning curve for adopting new implementation for different indexes.
Importantly, each pipeline module and its corresponding steps are
implemented as standalone functions, providing great flexibility for
anlaysis the index. Analysts can rearrange the order in the pipeline or
insert new step to experiment with possible new definition for the index
or to understand index behaviour.

\hypertarget{sec-index-pipeline}{%
\section{Details of the index pipline}\label{sec-index-pipeline}}
...
\textbf{Input: } \(x(\mathbf{s};\mathbf{t})\): multivariate data with
temporal coordinates

\textbf{Output: } \(x(\mathbf{s};\mathbf{t}^\prime)\): multivariate data
with aggregated temporal axis
