...
\midrule()
\endhead
1 & Temporal processing & \(x(t)\) & \(f[x(.)]\) & \(x^*(t^\prime)\)
where \(t\prime \in D_{t^\prime}\) \\
...
where bbbbb \\
5 & Dimension reduction & \(\mathbf{x}(s;t)\) & aaaaa &
\(\mathbf{y}(s;t)\) where \(y \in \mathbb{R}^d\) and \(d < p\) \\
...
variable in a set of fixed locations, \(x(t)\), as the input. This step
\emph{transforms} the original series into a new series to incorporate
the temporal neighbouring effect: \(x^*(.) = f[x(.)]\). In temporal
processing, the temporal dimension \(t\) may also transition to
\(t^\prime \subseteq D_{t^\prime}\), allowing for the decomposition of
time into its constituent temporal components. For example, this can be
used to break down daily data into monthly. This gives the output of the
temporal processing module as \(x^*(t^\prime)\).

An example of temporal processing is in Standardized Precipitation Index
(SPI), where monthly precipitation is summed over a rolling window of
size \(k\), also known as the time scale. Assuming an original time
series indexed by \(t= 1, 2, \cdots, T\), the aggregated series, indexed
by \(t^\prime = 1, 2, \cdots, T-k+1\), can be written as
