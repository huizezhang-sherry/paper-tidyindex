...
software design
\end{keywords}
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[borderline west={3pt}{0pt}{shadecolor}, sharp corners, frame hidden, interior hidden, boxrule=0pt, enhanced, breakable]}{\end{tcolorbox}}\fi
...
analysing specific index examples, we find indexes are often not just a
simple weighted linear aggregation of variables. When indexes have a
spatial and/or temporal component, different choices on aggregating
space and time will introduce uncertainty. {[}There is always new
variables found to be related to make the decision and should be
introduced into the index. The decision made in pre-processing data
coming from different sources such as satellite imageries, in-situ
stations, and large physical models.{]} While numerous indexes are
proposed in the literature, the well-known ones tend to prefer a simple
procedure: they may only consider one or two variables, i.e.~SPI or use
a decision-tree based process to incorporate multiple variables,
i.e.~(How El Nino is monitored by major climate institutes). More
complex indexes often have difficulties to reach consensus, due to their
uncertainty when applied in different scenarios.
...
beneficial?}/\textbf{who would benefit from this paper}

Typical ways to evaluate indexes is to compare it with the standard/
well-known indexes, such as SPI or SPEI for drought indexes, or a
response variable relevant for the monitoring purpose, i.e.~crop yield
for monitoring agricultural drought. While these evaluations show the
accuracy of the index to capture, they doesn't address the robustness of
the index,{[} given variation of the input parameters, or a change of
different study areas or across time.{]} To evaluate the robustness of
indexes, indexes need to be broken down into their fundamental building
blocks, and then analyse the effect of each component. The OECD handbook
(OECD, European Union, and Joint Research Centre - European Commission
2008) has recommended a set of guidelines for construct composite
indexes for measuring country's socio-economic performance. This paper
identifies a more general pipeline framework from which indexes can be
built from. Based on the pipeline, an R package, tidyindex, is developed
to {[}validate the indexes{]}. This work provides researchers actively
developing new indexes with tools to evaluate the proposed indexes. It
also helps index analysts to identify weakness in existing indexes for
...
\leavevmode\vadjust pre{\hypertarget{ref-ndvi}{}}%
