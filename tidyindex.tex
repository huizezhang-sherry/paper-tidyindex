...
\(\mathbf{x}(s;t)\), where
\(\mathbf{x} \subseteq \mathbb{R}^m \times \mathbb{R}^n\), or a subset
of variables in \(\mathbf{x}(s;t)\), as the input. It summarises the
high-dimensional information into a lower-dimension representation
\(\mathbf{y}(s;t)\), where \(\mathbf{y} \subseteq \mathbb{R}^d\) and
\(d < p\), as the output. The transformation can be based on
