...
3 & Variable transformation & \(x^i(s; t)\) & \(T[x(s;t)]\) &
\(Tx^i_t(s)\) ???? \\
4 & Scaling & \(x^i(s; t)\) & \([x(s;t) - \alpha]/\gamma\) &
\(Sx^i(s;t)\) ???? \\
...
6 & Distribution fit & \(x^i(s; t)\) & \(F[x(s;t)]\), where
\(F_{\mathbf{\theta}}(.)\) is the CDF function & \(p(s;t)\), where
\(p(.) \in [0, 1]\) is a probability value \\
7 & Normalising & \(x^i(s; t)\) & \(\Phi^{-1}[x(s; t)]\) & \(z(s; t)\),
where \(z\) has a normal distribution \\
8 & Benchmarking & \(x^i(s; t)\) & \(u[x(s;t)]\) & \(b(s;t)\) \\
9 & Simplification & \(x^i(s; t)\) & ccccc &
...
temporal dimension, \(x(s)\), as input. This step transforms the
...
\(\mathbf{x}(s;t)\), where \(\mathbf{x} \subseteq \mathbb{R}^p\) , as
the input. It summarises the high-dimensional information into a lower
dimension representation \(\mathbf{y}(s;t)\), where
\(y \subseteq \mathbb{R}^d\) and \(d < p\), as the output. The
transformation can be based on domain-specific knowledge, originating
from theories describing the underlying physical processes, or guided in
...
(Moved from the pipeline module to here) When a physical process is
absent to describe the phenomenon, multivariate data are often combined
using statistical methods, which can be further classified into linear
and non-linear techniques. Many social indexes use a linear combination
in an additive structure,
