...
the other hand, are concatenated to establish a sequential process
through which the tidy data object flows, creating a pipeline for data
processing. In this workflow, all the functions take inputs and outputs
as tidy data, enabling direct concatenation of functions and saving
analysts from bridging the outputs to subsequent operations.
Additionally, the pipeline workflow breaks down complex problems into
smaller, more manageable pieces. In an interactive data analysis, this
flexibility to link various operations together as needed makes the
pipeline workflow preferable over a monolithic design, where all the
steps are predetermined and tightly integrated into a single piece.

Dating back to earlier times, the pipeline approach to structure data
analysis draws inspiration from the interactive graphics literature
(Buja et al. 1988; Sutherland et al. 2000; Xie, Hofmann, and Cheng 2014;
Wickham et al. 2009). More recently, the tidy framework has provided a
conceptual guide for the tidyverse suite (Wickham et al. 2019) for
general-purpose data wrangling, a concept that has gained popularity
within the R community. Subsequently, more packages have since been
developed to analyze data using the tidy framework for specific domains,
for example, \texttt{tidytext} for text data (Silge and Robinson 2016),
and \texttt{naniar} for missing data (Tierney and Cook 2023), and
\texttt{tidymodels} for building machine learning models (Kuhn and
Wickham 2020). To create a tidy workflow tailored to a specific domain,
developers first need to identify the fundamental building blocks in the
typical workflow. These components are then implemented as modules,
which can be combined to form the pipeline. For example, in supervised
machine learning models, steps such as data splitting, model training
and model evaluation are commonly used in most workflow. In the
\texttt{tidymodels}, these steps are correspondingly implemented as
package \texttt{rsample}, \texttt{parsnip}, and \texttt{yardstick},
agnostic to the specific model chosen.
...
\leavevmode\vadjust pre{\hypertarget{ref-silge_tidytext_2016}{}}%
Silge, Julia, and David Robinson. 2016. {``Tidytext: {Text} {Mining} and
{Analysis} {Using} {Tidy} {Data} {Principles} in {R}.''} \emph{The
...
\leavevmode\vadjust pre{\hypertarget{ref-tierney_expanding_2023}{}}%
Tierney, Nicholas, and Dianne Cook. 2023. {``Expanding {Tidy} {Data}
{Principles} to {Facilitate} {Missing} {Data} {Exploration},
{Visualization} and {Assessment} of {Imputations}.''} \emph{Journal of
Statistical Software} 105 (February): 1--31.
