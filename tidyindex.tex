...
where \(x^l\) is the \(p\)th variable, \(s\) is the spatial location,
...
\(M = 1, \cdots, 12\). To simplify the notation, a subscript letter
\(s\) or \(t\) is used to denote the data have a fixed spatial locations
or temporal order. When the spatial locations are fixed, the data become
multivariate time series, denoted as \(x^l_s(t)\). Similarly,
\(x^l_t(s)\) represents multivariate spatial data when the temporal
dimension is fixed.
